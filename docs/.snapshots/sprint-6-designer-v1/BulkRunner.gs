...
function BulkRunner_runForRows(rows, contextLabel) {
  if (!rows || rows.length === 0) {
    SpreadsheetApp.getActiveSpreadsheet().toast('チェック対象がありません', 'メール認証チェック', 5);
    return;
  }
  ...
  // 開始 toast（処理時間の目安を併記。1行あたり約2秒で概算）
  var estimateMin = Math.max(1, Math.ceil(validRows.length * 2 / 60));
  SpreadsheetApp.getActiveSpreadsheet().toast(
    contextLabel + ' チェック開始：' + validRows.length + ' 件（目安 約 ' + estimateMin + ' 分）' + (dryRun ? '【DRY_RUN】' : ''),
    'メール認証チェック',
    6
  );
  ...
  for (var b = 0; b < validRows.length; b += BATCH_SIZE) {
    ...
    if (elapsedSec + timeBufferSec >= maxExecutionSec) {
      ...
      SpreadsheetApp.getActiveSpreadsheet().toast(
        '時間切れで一時停止しました（残り ' + unprocessedRows.length + ' 件）。もう一度メニューから実行すると、続きから自動で再開します。',
        'メール認証チェック',
        15
      );
      return;
  ...
  // 完了 toast（全 N 件 + 内訳。社長視点で「何件中いくつ成功したか」が一目で分かる形式）
  var processed = stats.ok + stats.warn + stats.ng + stats.retry;
  var msg = '完了 ' + processed + ' 件 ／ '
    + '✅ ' + stats.ok + ' ・ ⚠️ ' + stats.warn + ' ・ ❌ ' + stats.ng + ' ・ 🔁 ' + stats.retry;
  if (stats.skipped > 0) {
    msg += '（スキップ ' + stats.skipped + ' 件）';
  }
  SpreadsheetApp.getActiveSpreadsheet().toast(msg, 'メール認証チェック', 12);
  Logger.log('処理完了: ' + msg);
}
